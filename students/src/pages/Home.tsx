...
import { studentDetailsUrl, courseFetchUrl, subjectFetchUrl } from '../Config';
...
const StudentProfile: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        // Fetch and filter subjects using subjectIds
        if (response.data && response.data.subjectIds && response.data.subjectIds.length > 0) {
          fetchSelectedSubjects(response.data.subjectIds, token);
        }
      } catch (error) {
    ...
    const fetchSelectedSubjects = async (subjectIds: string[], token: string | null) => {
      try {
        const response = await axios.post(subjectFetchUrl, { subjectIds: subjectIds }, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 p-4 max-w-5xl mx-auto overflow-auto">
        ...
        <div className="flex-1 bg-white/80 rounded-2xl shadow-lg p-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            <span className="font-semibold text-indigo-700">Enrollment No:</span><span className="font-mono bg-indigo-50 px-2 py-0.5 rounded">
              {studentData.enrollmentNumber || 'Not Assigned'}
            </span>
      ...
      {studentData.educationalQualifications.length > 0 && (<div className="bg-white/80 border rounded-2xl shadow p-6 mt-8">
        <h2 className="text-lg font-bold border-b pb-2 mb-3 text-blue-900 flex items-center gap-2">
      ...
      {/* Documents Section */}
      {studentData.documents.length > 0 && (<div className="bg-white/80 border rounded-2xl shadow p-6 mt-8">
        <h2 className="text-lg font-bold border-b pb-2 mb-3 text-blue-900 flex items-center gap-2">
